...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  y_hat = softmax(np.dot(outside_vectors, np.reshape(center_word_vec,(len(center_word_vec),1))).T)[0]
  y = np.zeros(y_hat.shape)
  y[outside_word_idx] = 1
  loss = -np.log(y_hat)[outside_word_idx]
  grad_center_vec = np.dot(outside_vectors.T,(y_hat - y).reshape(len(y_hat),1)).reshape(len(outside_vectors.T),)
  grad_outside_vecs = (center_word_vec.reshape(len(center_word_vec),1)*(y_hat - y).reshape(1,len(y_hat))).T
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
    ...
    grad_center_vecs[word2ind[current_center_word]] = grad_center_vecs[word2ind[current_center_word]] + gradc
