...
word_vectors = sgd(
    ...
    word_vectors, 0.3, 6000, None, False, PRINT_EVERY=100)
...
#save word vector
words = tokens.keys()
idx = [tokens[word] for word in words]
vecs = word_vectors[idx, :]
vectors =  {word: list(vec) for word, vec in zip(words, vecs)}
vectors_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "vectors.json")
dump(vectors, vectors_path)
'''
